...
public class Main {
    public static void main(String[] args) {
        ...
        if (hiddenWord.contains("_")) {
            System.out.println("Sorry, you run out of guesses, the word was: " + wordToGuess);
        } else {
    ...
    public static char scanLetter(Scanner scanner) {
        ...
        while (true) {
            try {
                ...
                if (line.length() != 1) {
                    throw new Exception("Line length is not 1. Please enter a singel character");
                }
                guess = line.charAt(0);
                if (!Character.isLetter(guess)) {
                    throw new Exception("Exception is not a letter. Please enter a single character");
                }
                break;
            } catch (Exception e) {
